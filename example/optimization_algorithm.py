...
class GeneticAlgorithm:
    ...
    def create_individual(self):
        ...
        individual = self.create_random_matrix('random', density)
        return individual
    ...
    def predict_s_params(self, matrix: np.ndarray, frequency: float) -> Dict[str, float]:
        """预测矩阵在指定频率下的S参数"""
        # 归一化频率 (0-30GHz -> 0-1)
        frequency_norm = frequency / 30.0
        
        # 使用模型管理器进行预测
        s_params = self.model_manager.predict_single_sample(matrix, frequency)
        return s_params
    ...
    def fitness_function(self, matrix: np.ndarray, frequency: float, 
        ...
        return normalized_fitness, prediction_info
    ...
    def mutate(self, matrix: np.ndarray, mutation_rate: float = None) -> np.ndarray:
        ...
        # 额外的变异操作（以较低概率）
        if random.random() < 0.3:  # 30%概率进行额外变异
    ...
    def optimize(self, target_freq: float, target_params: Dict[str, Dict],
        ...
        # 进化循环
        for generation in tqdm(range(generations), desc="进化", disable=not verbose):
            # 计算适应度
            fitness_scores = []
            predictions_info = []
    ...
    def visualize_results(self, best_matrix: np.ndarray, 
        ...
        axes[0, 0].set_title(f'优化得到的矩阵 (密度: {np.mean(best_matrix):.3f})')
        axes[0, 0].set_xlabel('X')
        axes[0, 0].set_ylabel('Y')
        axes[0, 0].grid(True, alpha=0.3)
        
        # 2. 进化过程
        axes[0, 1].plot(fitness_history, label='平均适应度', alpha=0.7)
        axes[0, 1].plot(best_fitness_history, label='最佳适应度', linewidth=2)
        axes[0, 1].set_xlabel('代数')
        axes[0, 1].set_ylabel('适应度')
        axes[0, 1].set_title('进化过程')
        axes[0, 1].legend()
        ...
        axes[1, 1].axis('off')
        axes[1, 1].text(0.1, 0.5, f"最佳适应度: {best_fitness_history[-1]:.4f}\n"
                        f"最终密度: {density:.3f}\n"
                        f"总进化代数: {len(fitness_history)}", 
