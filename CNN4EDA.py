...
def train_model_with_resume(model, train_loader, val_loader, epochs=500, learning_rate=0.001, 
    ...
    try:
        for epoch in range(start_epoch, epochs):
            ...
            if epoch % 100 == 0 or epoch == epochs - 1:
                ...
                save_checkpoint(model, optimizer, epoch, train_losses, val_losses, checkpoint_path)
        ...
        # 保存中断时的模型状态
        checkpoint_path = os.path.join(checkpoint_dir, f'interrupted_epoch_{epoch}.pth')
        save_checkpoint(model, optimizer, epoch, train_losses, val_losses, checkpoint_path)
...
class GeneticAlgorithm:
    ...
    def optimize(self, target_freq, target_Leff, target_Q=None, 
                 population_size=4096, generations=100, 
...
def main():
    ...
    train_losses, val_losses = train_model_with_resume(
        ...
        epochs=1000, 
        learning_rate=0.001,
        checkpoint_dir='inductor_checkpoints',
        resume=True  # 设置为True以从上次训练点继续
    )
    
    # 3. 保存模型
    final_model_path = 'inductor_model_final.pth'
    torch.save({
        'model_state_dict': model.state_dict(),
        'X_scaler': X_scaler,
        'y_scaler': y_scaler,
